...
glusterd_op_stage_barrier (dict_t *dict, char **op_errstr)
{
        ...
        gf_boolean_t         barrier     = _gf_false;
        ...
        ret = dict_get_str (dict, "volname", &volname);
        if (ret) {
                gf_log (this->name, GF_LOG_ERROR, "Volaname not present in "
                        "dict");
                goto out;
        }
...
glusterd_op_barrier (dict_t *dict, char **op_errstr)
{
        ...
        copy = gf_strdup (barrier_op);
        if (!copy) {
                ret = -1;
                gf_log (this->name, GF_LOG_ERROR, "Failed to dup barrier_op");
                goto out;
        }

        ret = dict_set_dynstr (vol->dict, "features.barrier", copy);
        if (ret) {
                GF_FREE (copy);
